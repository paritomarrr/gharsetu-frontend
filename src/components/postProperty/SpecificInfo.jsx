...
import { Cable, Cctv, CircleParking, Dumbbell, Fence, HandPlatter, Phone, 
         SquareChevronUp, Trash2, Volleyball, WashingMachine, Waves, Webcam, Wifi } from 'lucide-react';
...
const SpecificInfo = () => {
  ...
  const [latitude, setLatitude] = useState(propertyForm.coordinates.latitude || '');
  const [longitude, setLongitude] = useState(propertyForm.coordinates.longitude || '');
  const [images, setImages] = useState([]);

  console.log('Imahes', images)

  console.log('Property Form 33', propertyForm);
  ...
  const flatFurnishings = [
    ...
    { name: 'Dining Table', icon: <WashingMachine size={18} />, value: 'diningTable' },
    { name: 'Washing Machine', icon: <HandPlatter size={18} />, value: 'washingMachine' },
    { name: 'Microwave', icon: <HandPlatter size={18} />, value: 'microwave' },
    { name: 'Stove', icon: <HandPlatter size={18} />, value: 'stove' },
    { name: 'Refrigerator', icon: <HandPlatter size={18} />, value: 'refrigerator' },
    { name: 'Water Purifier', icon: <HandPlatter size={18} />, value: 'waterPurifier' },
      ...
      <VStack spacing={9} align="start">
          ...
          <HStack spacing={8}>
            {propertySubTypes.map((subType) => (
              <Button
                ...
                fontWeight="semibold"
                borderRadius="xl"
                onClick={() => dispatch(updatePropertyForm({ propertySubType: subType }))}
        ...
        <VStack align="start" spacing={1}>
          <Input
            ...
            onChange={(e) => handleAddressChange('buildingProjectSociety', e.target.value)}
          />
          <Text color="gray.500" fontSize="xs">
            This helps us display your listing to the right audience based on location.
          </Text>
        ...
        {propertyForm.propertySubType === 'Plot' ? (
          ...
          <>
            <VStack align="start" spacing={4}>
              <Text fontSize="xl" fontWeight="semibold">Furnish Type:</Text>
              <HStack spacing={8}>
                {furnishTypes.map((type) => (
                  <Button
                    ...
                    fontWeight="semibold"
                    borderRadius="xl"
                    onClick={() => dispatch(updatePropertyForm({ furnishType: type }))}
                    colorScheme={propertyForm.furnishType === type ? 'teal' : 'gray'}
                  >
                    {type}
                  </Button>
                ))}
            ...
            <VStack align="start" spacing={4}>
              <Text fontSize="xl" fontWeight="semibold">BHK Configuration:</Text>
              <HStack spacing={8}>
                {bhkOptions.map((option) => (
                  <Button
                    ...
                    fontWeight="semibold"
                    borderRadius="xl"
                    onClick={() => dispatch(updatePropertyForm({ bhkConfig: option }))}
                    colorScheme={propertyForm.bhkConfig === option ? 'teal' : 'gray'}
                  >
                    {option}
                  </Button>
                ))}
          ...
          <HStack spacing={8}>
            {constructionTypes.map((type) => (
              <Button
                ...
                fontWeight="semibold"
                borderRadius="xl"
                onClick={() => dispatch(updatePropertyForm({ propertyStatus: type }))}
                colorScheme={propertyForm.propertyStatus === type ? 'teal' : 'gray'}
              >
                {type}
              </Button>
            ))}
        ...
        <VStack align="start" className="w-full">
          ...
          <div className="h-96 w-full max-w-[190vh]">
            <PostPropertyMap setLatitude={setLatitude} setLongitude={setLongitude} />
          </div>
        </VStack>
      </VStack>
    </Box>
